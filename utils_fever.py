...
class FeverProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, task):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, f"{task}.dev.jsonl")), "dev")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            elif 'evidence_sentence' in line: # symmetric
                text_b = line['evidence_sentence']
            else: # fm2
                text_b = ' '.join([e['text'] for e in line['gold_evidence']])
...
processors = {
    ...
    "symmetric": FeverProcessor,
    "adversarial": FeverProcessor,
    "fm2": FeverProcessor,
}
