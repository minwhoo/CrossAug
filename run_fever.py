            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate(args, model, tokenizer, during_training=True)
...
def evaluate(args, model, tokenizer, prefix="", during_training=False):
    ...
    elif args.task_name == "fever":
        eval_task_names = ('fever',) if during_training else args.eval_task_names
        eval_outputs_dirs = (args.output_dir + 'fever',) if during_training else tuple(args.output_dir + name for name in args.eval_task_names)
    else:
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                print("  {} = {:.4f}".format(key, result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if args.task_name == 'fever' and not evaluate:
        task = args.train_task_name
...
def main():
    ...
    parser.add_argument("--train_task_name", default=None, type=str, help="Train dataset name")
    parser.add_argument("--eval_task_names", nargs='+', help="List of eval task names")
